...
unsigned int CalculateTimerReload(unsigned int freq) {
    unsigned long clock = 11059200UL; // 加UL强制为32位无符号长整型
    unsigned long reload = clock / (12UL * 2UL * freq); // 避免中间结果溢出
    return (unsigned int)(65536UL - reload); // 转换为16位整数
}
